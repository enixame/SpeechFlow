...
public class SpeechSegmenterTests
{
    ...
    public async Task TestSpeechSegmenter_AccumulatesSamplesCorrectly()
    {
        ...
        mockVoiceFilterDetector.Setup(v => v.IsHumanVoice(It.Is<short[]>(s => s.Sum(x => (int)x) > 0))).Returns(true);  // Parole détectée
        mockVoiceFilterDetector.Setup(v => v.IsHumanVoice(It.Is<short[]>(s => s.Sum(x => (int)x) <= 0))).Returns(false);  // Parole non détectée
